...
for /f "eol=- tokens=1,2 skip=2" %%i in ('powershell -NoProfile gcim Win32_DiskDrive -f """%~1""" -p Index^,Partitions ^^^| select Index^,Partitions ^^^| sort ^^^| ft') do (
...
set %~2=&
for /f "tokens=1 delims=:" %%l in ('powershell -NoProfile ^(gcim Win32_DiskPartition -f """DeviceID='%~1'""" ^^^| Get-CimAssociatedInstance -ResultClassName Win32_LogicalDisk^).Name 2^> nul') do set "%~2=%%~l"& exit /b 0
exit /b -999

:getPartitionName <%1 = drive letter> <%2 = [out] disk 0-based index> <%3 = [out] disk partition 0-based index> <%4 = [out] disk partition name>
set %~2=& set %~3=& set %~4=&
for /f "tokens=2,4 delims=#," %%i in ('powershell -NoProfile ^(gcim Win32_LogicalDisk -f """DeviceID='%~1:'""" ^^^| Get-CimAssociatedInstance -ResultClassName Win32_DiskPartition^).Name 2^> nul') do (
	set "%~2=%%~i"
	set "%~3=%%~j"
...
for /f "eol=- tokens=1,2 skip=2" %%i in ('powershell -NoProfile ^(%gps%^).ParentProcessId ^^^| ForEach-Object { ^(%gps:PID=_%^).ParentProcessId } ^^^| ForEach-Object { ^(%gps:PID=_%^) ^^^| ft ParentProcessId^,ProcessId }') do call :log [PROCESS ID: %%~j] [PARENT PROCESS ID: %%~i]& set "pid=%%~j"
call :return pid
endlocal
exit /b

:getVolumeId <%1 = drive letter> <%2 = volume Id>
for /f "eol=- tokens=1,2 skip=2" %%i in ('powershell -NoProfile gcim Win32_Volume -f """DriveLetter='%~1:'""" -p DeviceID ^^^| ft DeviceID') do (
	set "%~2=%%~i" & set "%~2=!%~2:\=\\!"
	exit /b 0
)
exit /b -999
...
powershell -NoProfile (gcim Win32_Volume -f """DriveLetter='%~1:'""" -p DriveLetter ^| scim -Arguments @{ DriveLetter=$NULL } -PassThru).DriveLetter | findstr /ixl "%~1:" > nul || exit /b 0
exit /b -999
...
powershell -NoProfile (gcim Win32_Volume -f """%~1""" -p DriveLetter ^| scim -Arguments @{ DriveLetter="""%~2:""" } -PassThru).DriveLetter | findstr /ixl "%~2:" > nul && exit /b 0
exit /b -999
